...
class TaskQuery {
  ...
  TaskQuery copyWith({int? page, String? q, String? view}) => TaskQuery(
        view: view ?? this.view,
        deptId: deptId,
...
class TasksRepository {
  ...
  Future<Task> create(Map<String, dynamic> payload) async =>
      _task(await _api.post('/tasks', body: payload));
  ...
  Future<Paginated<ThreadEntry>> thread(int id,
    ...
    final body = await _api
        .get('/tasks/$id/thread', query: {'page': page, 'limit': limit});
  ...
  Future<Paginated<Attachment>> attachments(int id,
      {int page = 1, int limit = 25}) async {
    final body = await _api
        .get('/tasks/$id/attachments', query: {'page': page, 'limit': limit});
  ...
  Future<Task> transfer(int id, int deptId, {String? comments}) =>
      _task2(id, 'department',
  ...
  Future<Task> reply(
    ...
    return _task(await _api.upload(
      '/tasks/$id/reply',
      fields: {
        if (body != null) 'body': body,
  ...
  Future<List<Tag>> addTag(int id, {int? tagId, String? name}) async {
    final body = await _api.post('/tasks/$id/tags', body: {
  ...
  Future<List<TaskDependency>> addDependency(int id, int dependsOnId) async {
    final body = await _api
        .post('/tasks/$id/dependencies', body: {'depends_on_id': dependsOnId});
    return J.mapList(J.map(body)['data']).map(TaskDependency.fromJson).toList();
  }
